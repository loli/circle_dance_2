...
class AudioVisualizationBase:
    ...
    Handles audio analysis, parameter management, and event detection.
    """
    ...
    def __init__(self) -> None:
        ...
        # --- Shared Audio Parameters (Global) ---
        # These use the specialized graph-drawing parameter classes
        ...
        # Raw brightness from the receiver
        self.brightness = 0.0
    ...
    def update_tempo(self, new_bpm: float) -> None:
        """Normalizes BPM and applies smoothing."""
        if new_bpm <= 0:
            return

        # 1. Normalize BPM to a standard 'dance' range (80-160)
        while new_bpm < 80:
            new_bpm *= 2
        while new_bpm > 160:
            new_bpm /= 2

        # 2. Smooth the transition (Leaky Integrator)
        # This prevents the BPM value from jumping erratically
        self.bpm = (self.bpm * 0.95) + (new_bpm * 0.05)

    def process_audio_frame(self) -> list[dict]:
        """
        Processes raw audio into actionable events.
        Returns a list of triggered notes with their normalized energy.
        """
        self.receiver.update()
        self.brightness = self.receiver.brightness

        # 1. Handle Tempo Update
        if self.receiver.beat_detected:
            self.update_tempo(self.receiver.bpm)

        # 2. Note Extraction & Gating
        triggered_notes = []
        if self.receiver.notes_updated:
            # Noise Floor Gate: Ignore signal if overall volume is too low
            if self.receiver.rms >= self.noise_floor.value:
                new_e = self.receiver.notes
                current_peak = max(new_e) if any(new_e) else 0.0

                # Capture history for Delta (Attack) calculation
                self.old_energies = list(self.prev_energies)

                for i in range(12):
                    # Calculate 'Jolt' (Difference between frames)
                    attack = new_e[i] - self.old_energies[i]
                    ...
                    if is_attack and is_sens:
                        # Normalize energy for visual use (0.0 to 1.0)
                        # We use 0.8 as a typical max expected delta
                        norm_attack = max(0, min(1.0, (attack - self.attack_threshold.value) / 0.8))
                        triggered_notes.append(
                            {
                                "index": i,
                                "energy": norm_attack**2,  # Square for more dramatic impact
                            }
                        )

                self.prev_energies = new_e

        # 3. Update RMS History for HUD Graphs
    ...
    def draw(self, screen: pygame.Surface, font: pygame.font.Font) -> None:
        ...
        Guarantees the HUD is always rendered last.
        """
        # 1. Call the subclass-specific drawing logic
        self.render_visualization(screen, font)

        # 2. Assemble the drawing context for the HUD graphs
        context = {
