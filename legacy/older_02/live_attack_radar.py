...
pygame.display.set_caption("Interactive Radar - EDM Edition")
...
class NoteTrace:
    def __init__(self, note_index, angle, energy, decay_rate, inner_r, spacing, max_size, is_percussive=False):
        ...
        self.is_percussive = is_percussive
    ...
    def draw(self, surface, current_boost, lag_comp, global_brightness):
        ...
        # 5. Rendering Styles (HSS Logic)
        if self.is_percussive:
            # STYLE: Sharp Spiky Diamond for Percussion
            p_size = int(swell_size * 1.5)
            points = [(x, y - p_size), (x + p_size, y), (x, y + p_size), (x - p_size, y)]
            # Percussive hits are drawn slightly more opaque/white
            pygame.draw.polygon(surface, (*current_color, min(255, alpha + 50)), points)
            pygame.draw.circle(surface, (255, 255, 255, alpha), (int(x), int(y)), 2)
        else:
            # STYLE: Smooth Glowing Circle for Harmonic Synths
            ...
            pygame.draw.circle(
                note_surf, (*current_color, alpha), (surf_dim // 2, surf_dim // 2), int(swell_size // 1.5)
            )
...
class InteractiveStaff:
        ...
        try:
            while True:
                ...
                if isinstance(decoded, dict):
                    ...
                    # 2. Handle HPSS Analysis (Harmonic Notes + Percussion)
                    if "notes" in decoded:
                        new_e = decoded["notes"]
                        self.current_brightness = decoded.get("brightness", 0.0)
                        p_energy = decoded.get("percussion", 0.0)

                        self.peak_for_meter = max(new_e) if any(new_e) else 0.0

                        # --- Handle Harmonic Notes ---
                        if self.peak_for_meter >= self.noise_floor:
                            for i in range(12):
                                ...
                                if attack > self.attack_threshold and new_e[i] >= (
                                    ...
                                    norm_attack = (attack - self.attack_threshold) / 0.8
                        ...
                        # --- Handle Percussive Hits ---
                        if p_energy > self.attack_threshold * 0.4:
                            self.active_traces.append(
                                NoteTrace(
                                    6,
                                    self.scanning_angle,
    ...
    def draw(self):
        ...
        duck_offset = self.beat_boost * 15.0 if self.beat_pulse_enabled else 0.0

        for i in range(12):
            r = self.inner_radius + (i * self.ring_spacing) - duck_offset
        ...
        for t in self.active_traces:
            t.draw(screen, draw_boost, self.lag_comp, self.current_brightness)
        ...
        if self.show_help:
            ...
            screen.blit(hud_font.render("FLOOR", True, (255, 50, 50)), (m_x + m_w + 10, floor_y - 10))
        elif now < self.msg_timer:
            screen.blit(hud_font.render(self.msg, True, (255, 255, 255)), (20, 20))
