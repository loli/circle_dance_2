...
class InteractiveStaff:
    ...
    def capture_audio(self):
        try:
            while True:
                # Increased buffer to 2048 to ensure the larger JSON packet fits
                data, _ = sock.recvfrom(2048)
                decoded = json.loads(data.decode("utf-8"))

                if isinstance(decoded, dict):
                    # 1. Handle BPM (Beat) packets
                    if "bpm" in decoded:
                        self.update_tempo(decoded["bpm"])
                        if self.beat_pulse_enabled:
                            self.beat_boost = 1.0

                    # 2. Handle Analysis packets (Notes + Brightness)
                    elif "notes" in decoded:
                        new_e = decoded["notes"]
                        # Save the brightness for the playhead and traces
                        ...
                        for i in range(12):
                            ...
                            if attack > self.attack_threshold and new_e[i] >= (
                                ...
                                # Logic for scaling stays the same
